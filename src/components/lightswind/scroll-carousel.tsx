...
export const ScrollCarousel = forwardRef<HTMLDivElement, ScrollCarouselProps>(
  ({ features, className, maxScrollHeight }, ref) => {
    ...
    const renderFeatureCards = (featureSet: FeatureItem[], refs: React.MutableRefObject<HTMLDivElement[]>) =>
      featureSet.map((feature, index) => (
        <div
          ...
          z-10 gap-4  group relative transition-all duration-150 ease-in-out'
        >
          <div
            className={cn(
              `relative h-full p-4 lg:p-8 rounded-3xl backdrop-blur-sm 
              flex items-center justify-center z-10 
              transition-all duration-150 my-4`,
          ...
          >
            ...
            <div className='absolute bottom-4 z-10 w-full px-4'>
              <div
                className={cn(
                  `flex flex-col justify-end h-full opacity-100 translate-y-4 transition-all duration-150 ease-out text-center`
                )}
              >
                <h3 className='text-2xl mb-0 font-bold text-white transition-all duration-150'>{feature.title}</h3>
                <p className='text-white text-xs mb-4 opacity-60'>{feature.description}</p>
                {/* <img
                  src={feature.image}
                  alt=""
            ...
            <div className='pointer-events-none absolute inset-0 transition-all duration-150 group-hover:bg-black/5 dark:group-hover:bg-white/5 centered:bg-black/5 dark:centered:bg-white/5 rounded-2xl group-hover:blur-md' />
          </div>
        </div>
      ))

    return (
      <section className={cn('bg-transparent text-foreground relative overflow-hidden', className)} ref={ref}>
        <div
          ref={containerRef}
        ...
        >
          ...
          <div
            ...
            className='hidden xl:flex flex-col md:flex-row gap-8 items-center h-full px-6 md:px-0'
          >
          ...
          {isDesktop && (
            <div className='absolute bottom-10 left-1/2 -translate-x-1/2 w-64 h-2 bg-black/30 dark:bg-white/30 z-50 overflow-hidden rounded-full'>
              <div
                ref={progressBarRef}
                className='h-full rounded-full relative overflow-hidden transition-all duration-50'
                style={{ width: '0%' }}
              >
